import { requiredEnvVariable } from "./common/common";

export const region = requiredEnvVariable("REGION");
export const backupBucketName = requiredEnvVariable("BACKUP_BUCKET_NAME");
export const backupFilePath = requiredEnvVariable("BACKUP_FILE_PATH");
export const restoreTargetUserPoolId = requiredEnvVariable(
  "RESTORE_TARGET_USER_POOL_ID"
);

console.log("Backup Configuration:");
console.log("- region", region);
console.log("- backupBucketName", backupBucketName);
console.log("- backupFilePath", backupFilePath);
console.log("- restoreTargetUserPoolId", restoreTargetUserPoolId);
